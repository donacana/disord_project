...
async def ask_command(
    ...
    *,
    ...
    """FastAPI의 /ask API에 질문을 전달한다."""
    question = question.strip()

    if not question:
        await ctx.send("질문 내용을 입력해주세요.")
        return
    ...
    async with ctx.typing():
        try:
            ...
            await ctx.send("답변을 불러오는 중 오류가 발생했습니다.")
            return
...
async def health_command(ctx: commands.Context) -> None:
    ...
    try:
        ...
        await ctx.send(
            f"❌ 상태 확인 실패: HTTP {error.response.status_code}"
        )
        return
    except ValueError:
...
async def stats_command(ctx: commands.Context) -> None:
    ...
    try:
        ...
        await ctx.send(
            f"❌ 통계 조회 실패: HTTP {error.response.status_code}"
        )
        return
    except ValueError:
        await ctx.send("❌ 서버 응답 형식이 올바르지 않습니다.")
